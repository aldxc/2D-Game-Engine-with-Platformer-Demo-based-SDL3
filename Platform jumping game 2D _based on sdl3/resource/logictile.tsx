<?xml version="1.0" encoding="UTF-8"?>
<tileset version="1.10" tiledversion="1.12.1" name="logictile" tilewidth="16" tileheight="16" tilecount="6" columns="6" tilerendersize="grid">
 <image source="collision.png" width="96" height="16"/>
 <tile id="0">
  <properties>
   <property name="collision" type="int" value="2"/>
  </properties>
 </tile>
 <tile id="1">
  <properties>
   <property name="collision" type="int" value="1"/>
  </properties>
 </tile>
 <tile id="2">
  <properties>
   <property name="collision" type="int" value="3"/>
  </properties>
 </tile>
 <tile id="3">
  <properties>
   <property name="collision" type="int" value="4"/>
  </properties>
 </tile>
 <tile id="4">
  <properties>
   <property name="collision" type="int" value="5"/>
  </properties>
 </tile>
 <tile id="5">
  <properties>
   <property name="collision" type="int" value="6"/>
  </properties>
 </tile>
</tileset>
